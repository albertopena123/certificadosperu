...
export async function GET(
  ...
  try {
    ...
    // Don't allow preview certificates
    if (codigo.startsWith('PREVIEW-')) {
      return NextResponse.json(
        { error: 'Los certificados de vista previa no se pueden descargar' },
        { status: 400 }
      );
    }
    ...
    if (certificado.estado === 'ANULADO') {
      return NextResponse.json(
        { error: 'Este certificado ha sido anulado' },
        { status: 400 }
      );
    }
    ...
    return new NextResponse(new Uint8Array(pdfBuffer), {
      headers: {
        ...
        'Content-Disposition': `${disposition}; filename="certificado-${certificado.codigoVerificacion}.pdf"`,
      },
    });
  } catch (error) {
